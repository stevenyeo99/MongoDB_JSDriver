...
router.post('', (req, res, next) => {
  ...
  getDb().db().collection('products').insertOne(newProduct).then(result => {
    console.log(result);
...
router.patch('/:id', (req, res, next) => {
  ...
  getDb()
    ...
    .then(productData => {
      return res.status(200).json({ message: 'Product updated', productId: req.params.id });
    })
    .catch(err => {
